...
import { Lock, Upload, FileText, Download, Shield, Server, User, LogIn, Share2, Check, X } from "lucide-react";
...
const FileUpload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const encryptedBlob = new Blob([iv, rawAESKey, new Uint8Array(encryptedData)]);
  ...
  const handleDoctorDecrypt = async (file) => {
    try {
      ...
      const keyEntry = file.encryptedKeys?.find(k => k.doctorId === doctor.id);
      if (!keyEntry) throw new Error("No access to this file");
      
      // Decrypt the AES key
      const decryptedAESKey = await decryptWithRSA(doctor.privateKey, keyEntry.encryptedKey);
    ...
    try {
      if (status === 'approved') {
        ...
        const encryptedKeys = await Promise.all(selectedFileIds.map(async fileId => {
          ...
          if (!file) {
            console.error('File not found:', { fileId, availableFiles: uploadedFiles.map(f => f.ipfsHash) });
            throw new Error(`File not found: ${fileId}`);
          }

          const fileKeyStr = localStorage.getItem(`fileKey_${file.ipfsHash}`);
          if (!fileKeyStr) {
            throw new Error(`Encryption key not found for file: ${file.name}`);
          }
          ...
          try {
            ...
            const encryptedKey = await encryptWithRSA(request.doctorId.publicKey, keyArray);
        ...
        const response = await fetch(`https://advaya-maatrcare-node.onrender.com/patient/access-requests/${requestId}/respond`, {
          ...
          body: JSON.stringify({
            status,
            encryptedKeys: encryptedKeys.map(({ fileId, encryptedKey, iv }) => ({
              fileId,
              encryptedKey,
              iv
            }))
        ...
        setUploadedFiles(prev => {
          const updated = prev.map(file => {
            if (selectedFileIds.includes(file.ipfsHash)) {
              const encryptedKeyData = encryptedKeys.find(k => k.fileId === file.ipfsHash);
              if (encryptedKeyData) {
                const newKey = {
                ...
                const existingKeyIndex = file.encryptedKeys?.findIndex(k => k.doctorId === request.doctorId._id);
    ...
    <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white flex flex-col items-center px-4 py-12">
      <div className="w-full max-w-4xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          >
            {isDoctorMode ? <User className="w-5 h-5" /> : <LogIn className="w-5 h-5" />}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedDoctors([...e.target.selectedOptions].map(o => o.value))}
                >
                  {doctors.map(doctor => (
              ...
              {accessRequests.map(request => (
                <div key={request._id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-medium text-lg">Dr. {request.doctorId?.name}</h3>
                      <p className="text-sm text-gray-600">{request.doctorId?.email}</p>
                      <p className="text-sm text-gray-600">Specialization: {request.doctorId?.specialization}</p>
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="space-y-2">
                      {uploadedFiles.map(file => {
                        console.log('Rendering file:', { name: file.name, ipfsHash: file.ipfsHash }); // Debug log
                        return (
                          <label key={`${request._id}-${file.ipfsHash}`} className="flex items-center space-x-2">
                            <input
                              type="checkbox"
                              checked={selectedFiles[request._id]?.[file.ipfsHash] || false}
                              onChange={() => {
                  ...
                  <div className="flex justify-end space-x-3 mt-4 pt-4 border-t">
                    <button
                      onClick={() => handleAccessRequest(request._id, 'approved')}
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6">
          <h2 className="text-xl font-bold text-pink-700 mb-4 flex items-center gap-2">
            <FileText className="w-5 h-5" /> {isDoctorMode ? "Shared Files" : "Your Files"}
          </h2>
          <div className="space-y-3">
            {uploadedFiles?.filter(file => 
              isDoctorMode 
              ? file?.encryptedKeys?.some(k => k.doctorId === Number(selectedDoctorId))
              : true
            )?.map((file, i) => (
              <div key={i} className="bg-pink-50 rounded-lg p-4 border border-pink-100">
